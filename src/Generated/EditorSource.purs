    ...
    (lambda (input)
      (case input
        ...
        ('store-to-ephem untagged-store)
          (list 'output-ephem (alist-singleton 'step step-keypress))
...
;       (type-tag-add
;         store-tag
;         (alist-update-or-crash 'display f (type-tag-get store-tag store)))))
;
...
(define alist-update-or-crash
  'impl
    (lambda (k f alist)
      (case (alist-update-option k f alist)
        ('some a) a
        ('none)   (crash 'alist-update-or-crash 'key-not-found k))))

(define alist-update-option
  'examples
    (
      (alist-update-option 'a increment (alist-singleton 'a 1)) (some ((a 2)))
      (alist-update-option 'b increment (alist-singleton 'a 1)) (none)
    )
  'impl
    (lambda (k f alist)
      (let
        ; Using `loop` to short-circuit if we find the key,
        ; which something like `list-foldl` wouldn't.
        ((loop
          (lambda (acc xs)
            (if
              (list-empty? xs)
